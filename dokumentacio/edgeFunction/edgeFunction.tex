...
'keresztszorzata' a rájuk merőleges vektor $Z$ koordinátáját, hosszát adja meg.
\begin{align*}
...
A kapott $Z$ koordináta előjele a két vektor forgásától függ a geometriai
jobbkéz-szabály szerint.

\begin{center}
    % --- ROW 1: Figures 1 and 2 ---
    \begin{minipage}[t]{0.48\textwidth} % Figure 1: Alpha = 0
        \centering
        \begin{tikzpicture}[tdplot_main_coords,scale=1.5, line cap=round]
            ...
            \draw pic [opacity=0, angle radius=0.65cm] {angle = A--O--B};
        \end{tikzpicture}
        \vspace{0.5ex}
        % SHIFTED RIGHT: Adjusting the text position
        \hspace*{4mm}$\alpha = 0^\circ$
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth} % Figure 2: Alpha = 90
        \centering
        \begin{tikzpicture}[tdplot_main_coords,scale=1.5, line cap=round]
            \begin{scope}[canvas is xy plane at z=0]
                \draw[step=0.5, gray!30, very thin] (-1.5,-1.5) grid (1.5,1.5);
            ...
            \draw[->, blue, ultra thick] (O) -- (B) node[anchor=south west] {$\vec{u}$};
            \draw[->, green!80!black, ultra thick] (O) -- (C) node[anchor=south west] {$\mathbf{z}$};
            \draw pic ["$\alpha$", draw=black, angle radius=0.65cm] {angle = A--O--B};
        \end{tikzpicture}
        \vspace{0.5ex}
        % SHIFTED RIGHT: Adjusting the text position
        \hspace*{4mm}$\alpha = 90^\circ$
    \end{minipage}

    \vspace{1cm} % Vertical space between the two rows

    % --- ROW 2: Figures 3 and 4 (No change needed) ---
    \begin{minipage}[t]{0.48\textwidth} % Figure 3: Alpha = 180
        \centering
        \begin{tikzpicture}[tdplot_main_coords,scale=1.5, line cap=round]
            \begin{scope}[canvas is xy plane at z=0]
                \draw[step=0.5, gray!30, very thin] (-1.5,-1.5) grid (1.5,1.5);
    ...
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth} % Figure 4: Alpha = 270
        \centering
        \begin{tikzpicture}[tdplot_main_coords,scale=1.5, line cap=round]
            \begin{scope}[canvas is xy plane at z=0]
                \draw[step=0.5, gray!30, very thin] (-1.5,-1.5) grid (1.5,1.5);
